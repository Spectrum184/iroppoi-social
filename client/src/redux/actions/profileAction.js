...
export const getProfileUsers =
  ...
  async (dispatch) => {
    ...
    try {
      ...
      const resUser = getDataAPI(`/user/${id}`, auth.token);

      const resPost = getDataAPI(`/user-posts/${id}`, auth.token);

      const user = await resUser;

      const posts = await resPost;

      dispatch({ type: PROFILE_TYPES.GET_USER, payload: user.data });
